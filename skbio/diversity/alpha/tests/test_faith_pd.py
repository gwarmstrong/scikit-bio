...
from skbio.diversity.alpha import faith_pd, fast_faith_pd_prototype
...
class FastFaithPDTests(TestCase):
    ...
    def test_fast_faith_pd_prototype_none_observed(self):
        actual = faith_pd(np.array([], dtype=int),
                          np.array([], dtype=int),
                          self.t1)
        expected = 0.0
        self.assertAlmostEqual(actual, expected)
        actual = fast_faith_pd_prototype([0, 0, 0, 0, 0], self.oids1, self.t1, validate=True)
        expected = 0.0
        self.assertAlmostEqual(actual, expected)

    def test_fast_faith_pd_prototype_all_observed(self):
        actual = fast_faith_pd_prototype([1, 1, 1, 1, 1], self.oids1, self.t1)
        expected = sum(n.length for n in self.t1.traverse()
                       if n.length is not None)
        self.assertAlmostEqual(actual, expected)

        actual = fast_faith_pd_prototype([1, 2, 3, 4, 5], self.oids1, self.t1)
        expected = sum(n.length for n in self.t1.traverse()
                       if n.length is not None)
        self.assertAlmostEqual(actual, expected)
    ...
    def test_fast_faith_pd_prototype(self):
        ...
        # is a completely different implementation skbio's initial
        # phylogenetic diversity implementation
        actual = fast_faith_pd_prototype(self.b1[0], self.oids1, self.t1)
        expected = 4.5
        self.assertAlmostEqual(actual, expected)
        actual = fast_faith_pd_prototype(self.b1[1], self.oids1, self.t1)
        expected = 4.75
        self.assertAlmostEqual(actual, expected)
        actual = fast_faith_pd_prototype(self.b1[2], self.oids1, self.t1)
        expected = 4.75
        self.assertAlmostEqual(actual, expected)
        actual = fast_faith_pd_prototype(self.b1[3], self.oids1, self.t1)
        expected = 4.75
        self.assertAlmostEqual(actual, expected)

    def test_fast_faith_pd_prototype_extra_tips(self):
        # results are the same despite presences of unobserved tips in tree
        actual = fast_faith_pd_prototype(self.b1[0], self.oids1, self.t1_w_extra_tips)
        expected = fast_faith_pd_prototype(self.b1[0], self.oids1, self.t1)
        self.assertAlmostEqual(actual, expected)
        actual = fast_faith_pd_prototype(self.b1[1], self.oids1, self.t1_w_extra_tips)
        expected = fast_faith_pd_prototype(self.b1[1], self.oids1, self.t1)
        self.assertAlmostEqual(actual, expected)
        actual = fast_faith_pd_prototype(self.b1[2], self.oids1, self.t1_w_extra_tips)
        expected = fast_faith_pd_prototype(self.b1[2], self.oids1, self.t1)
        self.assertAlmostEqual(actual, expected)
        actual = fast_faith_pd_prototype(self.b1[3], self.oids1, self.t1_w_extra_tips)
        expected = fast_faith_pd_prototype(self.b1[3], self.oids1, self.t1)
        self.assertAlmostEqual(actual, expected)

    def test_fast_faith_pd_prototype_minimal(self):
        # two tips
        tree = TreeNode.read(StringIO('(OTU1:0.25, OTU2:0.25)root;'))
        actual = fast_faith_pd_prototype([1, 0], ['OTU1', 'OTU2'], tree)
        expected = 0.25
        self.assertEqual(actual, expected)

    def test_fast_faith_pd_prototype_qiime_tiny_test(self):
        # the following table and tree are derived from the QIIME 1.9.1
        # "tiny-test" data
        tt_table_fp = get_data_path(
            os.path.join('qiime-191-tt', 'otu-table.tsv'), 'data')
        ...
        expected = pd.read_csv(expected_fp, sep='\t', index_col=0)
        for sid in self.q_table.columns:
            actual = fast_faith_pd_prototype(self.q_table[sid],
                              otu_ids=self.q_table.index,
                              tree=self.q_tree)
            self.assertAlmostEqual(actual, expected['PD_whole_tree'][sid])

    def test_fast_faith_pd_prototype_root_not_observed(self):
        # expected values computed by hand
        tree = TreeNode.read(
            StringIO('((OTU1:0.1, OTU2:0.2):0.3, (OTU3:0.5, OTU4:0.7):1.1)'
                     'root;'))
        ...
        # root node not observed, but branch between (OTU1, OTU2) and root
        # is considered observed
        actual = fast_faith_pd_prototype([1, 1, 0, 0], otu_ids, tree)
        expected = 0.6
        self.assertAlmostEqual(actual, expected)

        # root node not observed, but branch between (OTU3, OTU4) and root
        # is considered observed
        actual = fast_faith_pd_prototype([0, 0, 1, 1], otu_ids, tree)
        expected = 2.3
        self.assertAlmostEqual(actual, expected)

    def test_fast_faith_pd_prototype_invalid_input(self):
        # tree has duplicated tip ids
        t = TreeNode.read(
            StringIO('(((((OTU1:0.5,OTU2:0.5):0.5,OTU3:1.0):1.0):0.0,(OTU4:'
                     '0.75,OTU2:0.75):1.25):0.0)root;'))
        counts = [1, 2, 3]
        otu_ids = ['OTU1', 'OTU2', 'OTU3']
        self.assertRaises(DuplicateNodeError, fast_faith_pd_prototype, counts, otu_ids,
                          t)
        ...
        counts = [1, 2, 3]
        otu_ids = ['OTU1', 'OTU2', 'OTU3']
        self.assertRaises(ValueError, fast_faith_pd_prototype, counts, otu_ids, t)
        ...
        counts = [1, 2, 3]
        otu_ids = ['OTU1', 'OTU2', 'OTU42']
        self.assertRaises(MissingNodeError, fast_faith_pd_prototype, counts, otu_ids, t)
